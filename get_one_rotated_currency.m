function croppedImage = get_one_rotated_currency(org_img)
I=double(org_img);
In=I; 
...
measurements = regionprops(BW, 'Orientation', 'BoundingBox');
angle = -measurements(1).Orientation;
croppedImage=imrotate(org_img, angle);
%targetSize = [360 700];
%l=size(croppedImage);
%r = centerCropWindow2d(l, targetSize);
%croppedImage = imcrop(croppedImage, r);
croppedImage = imcrop(croppedImage, [157 304 857 664]);
croppedImage = imcrop(croppedImage, [0 0 700 360]);
end